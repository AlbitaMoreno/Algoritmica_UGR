...
public:
    ...
    //añadade una ciudad al vector y sus distnacia
    ...
    bool operator<( const Solucion & s) const { //ordenados de menor a mayor
...
Solucion Branch_and_Bound(const vector< vector<int> > & matriz,const vector<int> & arista_menor,int tama)
{
    ...
    while ( !Q.empty() && (Q.top().CotaLocal() < CG) ) {
        ...
        for(int i=0; i<aux.size(); i++) {
            ...
            n_e.anadircidudadRestante(aux[i]);//añadimos la ultima que se quito
        }
    }
...
int main(int argc, char * argv[])
{
    ...
    vector< vector<int> >  matriz;//creamos una matriz con la distnacia entre ciudades
